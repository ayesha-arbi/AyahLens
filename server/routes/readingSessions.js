...
router.get('/', async (req, res) => {
  try {
    ...
    const data = await qfUserFetch('/auth/v1/reading-sessions', token);
    res.json(data);
...
router.post('/', async (req, res) => {
  try {
    const token = getUserToken(req);
    if (!token) return res.status(401).json({ error: 'User token required' });
    const data = await qfUserFetch('/auth/v1/reading-sessions', token, { method: 'POST', body: JSON.stringify(req.body) });
